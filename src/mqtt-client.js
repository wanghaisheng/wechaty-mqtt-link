...
async function send(params, bot) {
    ...
    if (params.messageType == 'Text') {
      ...
      "params":{
        ...
        if (params.messagePayload.url.indexOf('http') != -1 || params.messagePayload.url.indexOf('https') != -1) {
            msg = FileBox.fromUrl(params.messagePayload.url, { name: params.messagePayload.name })
        } else {
        ...
        if (params.messagePayload.url.indexOf('http') != -1 || params.messagePayload.url.indexOf('https') != -1) {
            ...
            msg = FileBox.fromFile(params.messagePayload.url)
        }
...
async function createRoom(params, bot) {
    ...
    for (let i in params.contactList) {
        let c = await bot.Contact.find({ id: params.contactList[i] })
        console.debug(c)
        contactList.push(c)
    }
